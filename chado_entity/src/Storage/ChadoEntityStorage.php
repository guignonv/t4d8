 ...
 *
 */
class ChadoEntityStorage extends SqlContentEntityStorage {
  ...
  public function hasData() {
    // Check if we are uninstalling the module. As chado is managed outside
    // Drupal, the ContentUninstallValidator class should not bother about
    // remaining data. So we trick it to make it think there are no data left.
    $backtrace = debug_backtrace();
    $calling_class = $backtrace[1]['class'] ?? '';
    if ($calling_class == \Drupal\Core\Entity\ContentUninstallValidator::class) {
      return FALSE;
    }
    // However we could always return FALSE in the sens there are no data
    // "under Drupal's management". Should we?
...
//    \Drupal::messenger()->addMessage('DEBUG ChadoEntityStorage::getCustomTableMapping'); //+debug
    ...
    $table_mapping = ChadoTableMapping::create($entity_type, $storage_definitions, $prefix);
  ...
  protected function loadFromSharedTables(array &$values, array &$translations, $load_from_revision) {
    ...
    if ($this->dataTable) {
      ...
      foreach ($result as $row) {
        ...
        //+val $langcode = empty($row[$this->defaultLangcodeKey]) ? $row[$this->langcodeKey] : LanguageInterface::LANGCODE_DEFAULT;
  ...
  protected function getQueryServiceName() {
    return 'chado_entity.database.entity.query.sql';
  }
  ...
  public function onEntityTypeDelete(EntityTypeInterface $entity_type) {
    // Do not delete biological schema tables: don't call parent method and do
    // nothing here.
  }
