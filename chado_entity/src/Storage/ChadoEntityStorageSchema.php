...
/**
 * Defines a schema handler that supports Chado entities.
 *
 * @see https://api.drupal.org/api/drupal/core%21lib%21Drupal%21Core%21Entity%21Sql%21SqlContentEntityStorageSchema.php/class/SqlContentEntityStorageSchema/9.3.x
 */
class ChadoEntityStorageSchema extends SqlContentEntityStorageSchema {
...
//\Drupal::messenger()->addMessage('DEBUG 3 ' . print_r($table, TRUE)); //+debug
  ...
  protected function getFieldSchemaIdentifierName($entity_type_id, $field_name, $key = NULL) {
    //+val $real_key = isset($key) ? "{$entity_type_id}_field__{$field_name}__{$key}" : "{$entity_type_id}_field__{$field_name}";
    $real_key = isset($key)
      ? "{$entity_type_id}_field__{$field_name}__{$key}"
      : $field_name
    ;
  ...
  protected function getFieldForeignKeys($field_name, array $field_schema, array $column_mapping) {
    ...
    foreach ($field_schema['foreign keys'] as $specifier => $specification) {
      ...
      // added to an entity type.
      $entity_type_id = $this->entityType->id();
      //+val $real_specifier = "{$entity_type_id}_field__{$field_name}__{$specifier}";
      $real_specifier = "{$field_name}__{$specifier}";
  ...
  protected function initializeDataTable(ContentEntityTypeInterface $entity_type) {
    ...
    $schema = [
      ...
      //+al 'primary key' => [$id_key, $entity_type->getKey('langcode')],
      'primary key' => [$id_key],
      'indexes' => [
        //+val $entity_type_id . '__id__default_langcode__langcode' => [$id_key, $entity_type->getKey('default_langcode'), $entity_type->getKey('langcode')],
      ],
      'foreign keys' => [
        $entity_type_id => [
          'table' => $this->storage->getBaseTable(),
  ...
  protected function processDataTable(ContentEntityTypeInterface $entity_type, array &$schema) {
    ...
    // performant.
    //+val $schema['fields'][$entity_type->getKey('default_langcode')]['not null'] = TRUE;
  }
