...
namespace Drupal\stockref_field\Plugin\Field\FieldFormatter;

use Drupal\Core\Cache\CacheableMetadata;
use Drupal\Core\Entity\EntityInterface;
use Drupal\stockref_field\Plugin\Field\StockReferenceFieldItemListInterface;
use Drupal\Core\Field\FieldItemListInterface;
use Drupal\stockref_field\Plugin\Field\FieldType\StockReferenceItem;
 ...
 */
abstract class StockReferenceFormatterBase extends FormatterBase {
   ...
   * directly.
   *
   * For each entity, the StockReferenceItem by which the entity is referenced
   ...
   * @see ::prepareView()
   */
  protected function getEntitiesToView(StockReferenceFieldItemListInterface $items, $langcode) {
    $entities = [];
  ...
  public function prepareView(array $entities_items) {
    ...
    foreach ($entities_items as $items) {
      foreach ($items as $item) {
        if (isset($target_entities[$item->object_id])) {
          $item->entity = $target_entities[$item->object_id];
          $item->_loaded = TRUE;
        }
   ...
   *
   * @param \Drupal\stockref_field\Plugin\Field\FieldType\StockReferenceItem $item
   *   The item to check.
   *
   * @return bool
   *   TRUE if the entity needs to be loaded.
   */
  protected function needsEntityLoad(StockReferenceItem $item) {
