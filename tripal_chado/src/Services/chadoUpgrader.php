...
class chadoUpgrader extends bulkPgSchemaInstaller {
  ...
  public function upgrade(
    ...
    if (!ChadoSchema::checkSchemaExists($chado_schema)) {
      throw new \Exception(
    ...
    $chado_schema_quoted = ChadoSchema::quotePgObjectId($chado_schema);

    $ref_chado_schema = $this::CHADO_REF_SCHEMA_13;
    $ref_chado_schema_quoted = ChadoSchema::quotePgObjectId($ref_chado_schema);
  ...
  protected function setupReferenceSchema(
    ...
    if (ChadoSchema::checkSchemaExists($ref_chado_schema)) {
      // Yes, check minimal structure.
      $schema =
        new ChadoSchema(NULL, $ref_chado_schema);
      if (!ChadoSchema::checkSchemaTableExists('chadoprop', $ref_chado_schema)) {
      ...
      ChadoSchema::createSchema($ref_chado_schema);
      ...
      if (!$success) {
        ...
        try {
          ChadoSchema::dropSchema($ref_chado_schema);
        }
        catch (Exception $e) {
          // Warn error in logs.
          $this->logger->error(
  ...
  protected function prepareDropColumnDefaults($parameters) {
    ...
    foreach ($tables as $table) {
      ...
      $table_raw_definition = ChadoSchema::getSchemaTableDdl($table, $chado_schema);
      $table_definition = ChadoSchema::parseTableDdl($table_raw_definition);
  ...
  protected function prepareUpgradeTables($parameters) {
    ...
    foreach ($old_tables as $old_table_name => $old_table) {
      ...
      if (array_key_exists($old_table_name, $chado_column_upgrade)) {
        // Get old table definition.
        $old_table_raw_definition = ChadoSchema::getSchemaTableDdl($new_table_name, $chado_schema);
        $old_table_definition = ChadoSchema::parseTableDdl($old_table_raw_definition);
    ...
    foreach ($new_tables as $new_table_name => $new_table) {
      ...
      $new_table_raw_definition = ChadoSchema::getSchemaTableDdl($new_table_name, $ref_chado_schema);
      $new_table_definition = ChadoSchema::parseTableDdl($new_table_raw_definition);
