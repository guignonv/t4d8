-- =============================================
-- Install PostgreSQL pg-clone-schema for Tripal
-- =============================================
-- This script originaly comes from
-- https://github.com/denishpatel/pg-clone-schema
-- and https://wiki.postgresql.org/wiki/Clone_schema (original version).
-- It has been modified for Drupal and Tripal integration.
--
-- Any SQL code added here must be able to be run several times without side
-- effects. For instance, use "CREATE OR REPLACE" or "IF NOT EXISTS" expressions
-- when possible.
-- IMPORTANT: Any feature added or modified here should also be removed by
-- the accompanying uninstallation script "clone_schema_uninstall.sql".
...
CREATE OR REPLACE FUNCTION public.tripal_get_table_ddl(
  in_schema varchar,
  in_table varchar,
  bfkeys  boolean
)
...
CREATE OR REPLACE FUNCTION public.tripal_clone_schema(
    source_schema text,
    dest_schema text,
...
-- SAMPLE CALL:
-- SELECT tripal_clone_schema('sample', 'sample_clone2', True, False);
...
-- Create tables including partitioned ones (parent/children) and unlogged ones.  Order by is critical since child partition range logic is dependent on it.
  ...
  LOOP
    ...
    IF relknd = 'r' THEN
      IF ddl_only THEN
        IF data_type = 'USER-DEFINED' THEN
          SELECT * INTO buffer3 FROM public.tripal_get_table_ddl(quote_ident(source_schema), tblname, False);
...
-- Create functions
  ...
  LOOP
    ...
    SELECT replace(
      'CREATE OR REPLACE FUNCTION '
        ...
        || ' LANGUAGE plpgsql AS $_$ BEGIN END; $_$;',
  ...
  LOOP
    ...
    SELECT
      ...
      || ') (sfunc = '
      || regexp_replace(a.aggtransfn::text, '(^|\W)' || quote_ident(source_schema) || '\.', '\1' || quote_ident(dest_schema) || '.', 'gis')
      || ', stype = '
      || format_type(a.aggtranstype, NULL)
      || CASE
  ...
  LOOP
    ...
    FOR column_, default_ IN
      SELECT column_name::text,
             regexp_replace(column_default::text, '(^|\W)' || quote_ident(source_schema) || '\.', '\1' || quote_ident(dest_schema) || '.', 'gis')
